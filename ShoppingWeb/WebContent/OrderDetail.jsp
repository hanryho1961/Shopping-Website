		...
		<div class="body">
			...
			<table style="border-top: 1px solid black; border-collapse: collapse; width: 1300px; margin-left: 20px">
				...
				<%
					...
					ArrayList<OrderDetails> list = orderDetailsModel.getList(Integer.parseInt(request.getParameter("orderid")));
					for (OrderDetails orderDetails: list) {
						sanPham = sanPhamModel.getSanPham(orderDetails.getMasp());
				%>
					<tr>
						<td><%=sanPham.getMasp() %></td>
						<td><%=sanPham.getTensp() %></td>
						<td><%=sanPham.getGia() %></td>
						<td><%=orderDetails.getSoLuong() %></td>
					</tr>
				<% } %>
			</table>
		</div>		
