	...
	<div class="introduction">
		...
		<div class="body">
			...
			<table style="border-top: 1px solid black; border-collapse: collapse; width: 1300px; margin-left: 20px">
				...
				<c:forEach var="items" items='<%=session.getAttribute("listitems") %>'>
					<tr>
						<td>${count}</td>
						<c:set var="count" value="${count + 1 }" scope="session" />
						<td>${items.sanPham.tensp }</td>
						<td>${items.soLuong }</td>
						<td>${items.sanPham.gia }</td>
						<td>${items.sanPham.gia * items.soLuong }</td>
						<td>
							<a href="CustomerPageServlet?task=delete&customername=<%=session.getAttribute("customername") %>&masp=${items.sanPham.masp}">Remove Product</a>
						</td>
						<c:set var="total" value="${total + items.sanPham.gia * items.soLuong  }" scope="session" />
					</tr>
				</c:forEach>
				<tr>
					<td></td>
		...
		<div class="button">
			<table>
				<tr>
					<td>
						<form action="CartPageServlet?task=continue&customername=<%=session.getAttribute("customername") %>" method="post">
							<input type="submit" value="Continue Buying" class="buy">
						</form>
					</td>
					<td>
						<form action="CustomerPageServlet?task=pay&customername=<%=session.getAttribute("customername") %>&total=${total }" method="post">
